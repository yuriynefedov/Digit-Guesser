...
from Sizing import The4xResizer
...
class NetWindow(QWidget):
    ...
    def editSize(self):
        print('in')
        initial = Image.new('RGB', (56,56), (255,255,255))
        xc = 0
        for x in self.pzone.pixlist:
                yc = 0
                for y in x:
                    #print(xc,yc)
                    if y.state == 1:
                        initial.putpixel((yc,xc), (0,0,0))
                    else:
                        initial.putpixel((yc,xc), (255,255,255))
                    yc += 1
                xc += 1

        initial.save('init.png')
        self.resizer.resize('init.png', 'final.png')
        final = Image.open('final.png')
        
        self.newpixels = []
        for x in range(28):
            self.newpixels.insert(x, [])
            for y in range(28):
                state = final.getpixel((x,y))
                if state == (255,255,255):
                    self.newpixels[x].insert(y, 0)
                else:
                    self.newpixels[x].insert(y, 1)
        #print(self.newpixels)

    def process(self):
            self.editSize()

            string = ''
            for row in self.newpixels: #####
                for pixel in row:
                    if pixel:
                        string += ',255'
                    else:
